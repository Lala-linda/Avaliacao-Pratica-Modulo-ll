...
def create_table(conn):
    with conn:
        conn.execute("CREATE TABLE alunos(id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT NOT NULL, idade INTEGER NOT NULL, email TEXT NOT NULL)")
...
def list_alunos(conn):
    with closing(conn.cursor()) as cur:
        cur.execute("SELECT nome, idade, email FROM alunos")
        rows = cur.fetchall()
        print("■ LISTAR TODOS")
        for r in rows:
            print(r)
...
def get_aluno_by_id(conn, aluno_id):
    with closing(conn.cursor()) as cur:
        cur.execute("SELECT nome, idade, email FROM alunos WHERE id = ?", (aluno_id,))
        row = cur.fetchone()
